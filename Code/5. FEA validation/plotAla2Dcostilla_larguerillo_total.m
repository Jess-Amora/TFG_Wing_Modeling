function plotAla2Dcostilla_larguerillo_total(avion)
...
% Flag graph
        ...
        % Cajón de torsión ala
        plot(linspace(0,Lf,100),c1*Distancia_larguero_posterior_cuerda_porcentaje*ones(100,1),'r','LineWidth',3, 'HandleVisibility', 'off');
        plot(linspace(0,Lf,100),c1*Distancia_larguero_anterior_cuerda_porcentaje*ones(100,1),'r','LineWidth',3, 'HandleVisibility', 'off');
        plot(zeros(100,1),linspace(Distancia_larguero_anterior_cuerda_porcentaje*c1 , c1*Distancia_larguero_posterior_cuerda_porcentaje,100),'r','LineWidth',3, 'HandleVisibility', 'off');
        
        % Cajón de torsión fuselaje
        plot(zeros(1,numero_de_puntos_en_las_lineas),linspace(Distancia_larguero_anterior_cuerda_porcentaje*c1,Distancia_larguero_posterior_cuerda_porcentaje*c1,numero_de_puntos_en_las_lineas),'r','LineWidth',3, 'HandleVisibility', 'off');
        plot(linspace(0,Lf,numero_de_puntos_en_las_lineas),c1*Distancia_larguero_posterior_cuerda_porcentaje*ones(1,numero_de_puntos_en_las_lineas),'r','LineWidth',3, 'HandleVisibility', 'off');
        plot(linspace(0,Lf,numero_de_puntos_en_las_lineas),c1*Distancia_larguero_anterior_cuerda_porcentaje*ones(1,numero_de_puntos_en_las_lineas),'r','LineWidth',3, 'HandleVisibility', 'off');
        ...
        for i=1:numero_costillas
            if i == 1
                plot(squeeze(costillas(i,1,:)),squeeze(costillas(i,2,:)),'k', 'DisplayName', 'Costillas');
            else
        ...
        for i=1:size(larguerillos,1)
            if i == 1
                ...
                plot(squeeze(larguerillos(i,1,:)),squeeze(larguerillos(i,2,:)),'k', 'HandleVisibility', 'off');
            end
        end
        ...
        for i=1:numero_larguerillo_total
            if i == 1
                plot(squeeze(larguerillos_fuselaje(i,1,:)),squeeze(larguerillos_fuselaje(i,2,:)),'k', 'DisplayName', 'larguerillos');
            else
    ...
    if isfield(avion.folder, 'figures') && ~isempty(avion.folder.figures)
        ...
        saveas(fig, fullfile(avion.folder.figures, 'Plot_costilla_larguerillo_total.png'));
        saveas(fig, fullfile(avion.folder.figures, 'Plot_costilla_larguerillo_total.fig'));
        disp(['📊 Saved plot to: ', fullfile(avion.folder.figures, 'Plot_costilla_larguerillo_total.png')]);
    end
