function plotAla2D_fuerzas_L_sust(avion, ax, closeflag)
    ...
    if nargin < 2 || isempty(ax)
        fig1 = figure('Name', 'Fuerzas L Sustentación', 'NumberTitle', 'off');
        ax = gca;
    else
    ...
    ala = avion.ala;
    stem3(ax, ala.x_L, ala.y_L, ala.L, 'filled', 'DisplayName', 'Fuerzas de Sustentación');
    view(ax, 3);

    xlabel(ax, 'X (Envergadura)');
    ylabel(ax, 'Y (Cuerda)');
    zlabel(ax, 'Magnitud de la Fuerza');
    title(ax, 'Distribución de Fuerzas de Sustentación en el Ala');
