function plotAla2D_costillas_larguerillos(avion, ax, closeflag)
    % plotAla2D_costillas_larguerillos: Generates a 2D plot of the wing showing
    % ribs (costillas) and stringers (larguerillos) along with the torsion box.
    %
    % Parameters:
    ...
    if nargin < 2 || isempty(ax)
        fig = figure('Name', 'Ala 2D - Costillas y Larguerillos', 'NumberTitle', 'off');
        ax = gca;
    else
    ...
    plotStructuralElements(ax, ala.larguerillos, 'Larguerillos', 'b');
    ...
    ylabel(ax, 'Y (Cuerda)');
    title(ax, 'Visualización de Costillas y Larguerillos en el Ala');
    legend(ax, 'Location', 'southeast');

    hold(ax, 'off');

    %% 💾 **Save Plot and Data**
    savePlot(fig, avion, 'Plot_costillas_larguerillos');
