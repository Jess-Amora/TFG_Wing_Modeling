function plotAla2D_V_n1(avion, ax, closeflag)
    ...
    if nargin < 2 || isempty(ax)
        fig1 = figure('Name', 'Fuerzas V N1', 'NumberTitle', 'off');
        ax = gca;
    else
    ...
    V_rear = avion.forces_n1.V.rear;
    V_front = avion.forces_n1.V.front;
    ...
    title(ax, 'Distribución de Fuerzas Cortantes V en el Ala (N1)');
    ...
    if isfield(avion.folder, 'figures') && ~isempty(avion.folder.figures)
        ...
        saveas(fig1, fullfile(avion.folder.figures, 'Plot_forces_V_n1.png'));
        saveas(fig1, fullfile(avion.folder.figures, 'Plot_forces_V_n1.fig'));
        disp(['📊 Saved plot to: ', fullfile(avion.folder.figures, 'Plot_forces_V_n1.png')]);
    end
