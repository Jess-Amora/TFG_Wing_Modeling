function plotAla2Dlarguerillo_fuselaje(avion, ax, closeflag)
    % 📈 Plot Larguerillos (Stringers) in Fuselaje
    %
    % Parameters:
    ...
    if nargin < 2 || isempty(ax)
        fig = figure('Name', 'Larguerillos en el Fuselaje', 'NumberTitle', 'off');
        ax = gca;
    else
    ...
    %% 🔩 **Draw Larguerillos (Stringers)**
    plotStringers(ax, fuselaje, ala);

    %% 📊 **Finalize Plot**
    xlabel(ax, 'X (Envergadura)');
    ylabel(ax, 'Y (Cuerda)');
    title(ax, 'Larguerillos en el Fuselaje');
    legend(ax, 'Location', 'southeast');

    hold(ax, 'off');

    %% 💾 **Save Plot and Data**
    savePlot(fig, avion, 'Plot_larguerillos_fuselaje');
...
function plotStringers(ax, fuselaje, ala)
    ...
    for i = 1:numero_larguerillo_total
        if i == 1
            plot(ax, squeeze(larguerillos_fuselaje(i, 1, :)), squeeze(larguerillos_fuselaje(i, 2, :)), ...
                 'b', 'DisplayName', 'Larguerillos');
        else
            plot(ax, squeeze(larguerillos_fuselaje(i, 1, :)), squeeze(larguerillos_fuselaje(i, 2, :)), ...
