function plotAla2D_V_n_lim(avion, ax, closeflag)
    ...
    if nargin < 2 || isempty(ax)
        fig1 = figure('Name', 'Fuerzas V N Lim', 'NumberTitle', 'off');
        ax = gca;
    else
    ...
    V_rear = avion.forces_n_lim.V.rear;
    V_front = avion.forces_n_lim.V.front;
    ...
    title(ax, 'Distribución de Fuerzas Cortantes V en el Ala (N Lim)');
    ...
    if isfield(avion.folder, 'figures') && ~isempty(avion.folder.figures)
        ...
        saveas(fig1, fullfile(avion.folder.figures, 'Plot_forces_V_n_lim.png'));
        saveas(fig1, fullfile(avion.folder.figures, 'Plot_forces_V_n_lim.fig'));
        disp(['📊 Saved plot to: ', fullfile(avion.folder.figures, 'Plot_forces_V_n_lim.png')]);
    end
