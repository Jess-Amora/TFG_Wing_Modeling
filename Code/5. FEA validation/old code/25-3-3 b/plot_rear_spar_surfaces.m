function plot_rear_spar_surfaces(combined_nodes, superficie_horizontal_larguero_posterior)
...
%   plotfilename: Path to save the resulting plot
    ...
    if isfield(avion.folder, 'Figures') && ~isempty(avion.folder.figures)
        ...
        saveas(fig, fullfile(avion.folder.figures, 'Plot_quad_rear_spar_2D_ala.png'));
        saveas(fig, fullfile(avion.folder.figures, 'Plot_quad_rear_spar_2D_ala.fig'));
        disp(['📊 Saved plot to: ', fullfile(avion.folder.figures, 'Plot_quad_rear_spar_2D_ala.png')]);
    end
