function plotAla2Dlarguerillo(avion)
    ...
    numero_costillas = avion.ala.numero_costillas;
...
% Flag graph
        ...
        % Ala
        % larguerillos = squeeze(ala.mesh.nodos_larguerillos);
        % larguerillos = larguerillos([1:2],:);
        larguerillos = ala.larguerillos;

        % % numero_larguerillo_total = size(larguerillos,1);
        %  plot(larguerillos(:,1),larguerillos(:,2),'k', 'DisplayName', 'larguerillos')
    ...
    if isfield(avion.folder, 'figures') && ~isempty(avion.folder.figures)
        ...
        saveas(fig1, fullfile(avion.folder.figures, 'Plot_larguerillos.png'));
        saveas(fig1, fullfile(avion.folder.figures, 'Plot_larguerillos.fig'));
        disp(['📊 Saved plot to: ', fullfile(avion.folder.figures, 'Plot_larguerillos.png')]);
    end
