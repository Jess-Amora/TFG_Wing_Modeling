function plotAla2D_mesh_nodos(avion)
...
% Flag graph
        ...
        %Ala larguerillo
        numero_larguerillo_total = size(ala.mesh.nodos_larguerillos,1);
        larguerillos = ala.mesh.nodos_larguerillos;
        ...
        % nodes = ala.mesh.nodos_intersecciones;
    ...
    if isfield(avion.folder, 'figures') && ~isempty(avion.folder.figures)
        ...
        saveas(fig, fullfile(avion.folder.figures, 'Plot_nodos.png'));
        saveas(fig, fullfile(avion.folder.figures, 'Plot_nodos.fig'));
        disp(['📊 Saved plot to: ', fullfile(avion.folder.figures, 'Plot_nodos.png')]);
    end
