...
datosEstructural = TFG_Amora.aviones.(name).datosEstructural;
cargas = TFG_Amora.aviones.(name).cargas;
ala = TFG_Amora.aviones.(name).ala;
...
% Ala 
larguerillos = avion.ala.mesh_struct.larguerillos;
numero_larguerillos_total = size(larguerillos,1);
linea_larguero_anterior = ala.geom_struct.linea_larg_anterior;
linea_larguero_posterior = ala.geom_struct.linea_larg_posterior;
numero_costillas = ala.ribs_struct.num_costillas_total;
numero_costillas_triangulo = ala.ribs_struct.num_costillas_tri;
% num_stringers_last_rib = ala.num_stringers_last_rib;
% nodos_larguerillos = ala.mesh.nodos_larguerillos;

%% Mesh ALA
% intersecciones_costillas_larguerillos_ala = ala.mesh_i.intersecciones_costillas_larguerillos;
...
index_larguerillos_anterior = ala.mesh_struct.index_larguerillos_anterior;
% inserted_nodes = ala.mesh_struct.inserted_nodes;
% nodos
index_larguerillos_anterior_ala = ala.mesh_struct.index_larguerillos_anterior; % Número de intersección que hace el larguerillo con la costillas y su punto medio (Punto medio entre costillas).
...
nodos_larguerillos = squeeze(ala.mesh_struct.nodos_larguerillos);
...
% plot_rear_spar_surfaces_v0(combined_nodes, superficie_horizontal_larguero_posterior,avion.folder);
...
% plot_stringer_regular_surfaces_v4(combined_nodes, quad_surfaces_regular, avion);
 OnlyPlotSurface_v7(combined_nodes, [], ...
    [quad_surfaces_regular],[superficie_horizontal_larguero_posterior], "Plot regular", fullfile(avion.folder.figures,'regular.png'),avion,datosEstructural);
...
for stringer_index = num_stringers_last_rib:max_stringer - 1
    ...
    % end_rib
    %% ✅ Process Up to the Second-to-Last Rib (Regular-like Behavior)
    [quad_irregular_stringer,tri_surfaces_stringer, warn, combined_nodes] = create_surfaces_for_stringer_irregular_v9( ...
        combined_nodes, stringer_index, end_rib, ala.geom_struct,avion.datosEstructural);
