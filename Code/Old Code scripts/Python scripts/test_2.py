...
def create_points(points, doc):
    """Create Part vertices for all points."""
    for i, pt in enumerate(points):
        vertex_shape = Part.Vertex(pt)  # Create a vertex shape
        vertex_obj = doc.addObject("Part::Feature", f"Vertex_{i}")  # Add to the document
...
def main():
    points_csv  = "..\output\points.csv"  # Path to the CSV file with points
    step_output = "..\\..\\Freecad\output_model.step"  # Output STEP file path

    points = read_points(points_csv)  # Read points from the CSV file

    doc = App.newDocument("PointsModel")  # Create a new FreeCAD document
    create_points(points, doc)  # Add points as vertices to the document
    export_to_step(doc, step_output)  # Export the points to a STEP file
