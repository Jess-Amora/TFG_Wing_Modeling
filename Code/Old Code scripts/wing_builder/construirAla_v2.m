...
function [results] = construirAla_v2(avion,datosEstructural,cargas)
...
% Parámetros de entrada:
    ...
    % Variables para Mesh
    nodos_posterior = zeros(2,(numero_costillas*2)-1); % Los nodos en el borde de salida que son numero_costillas (costillas) + numero_costillas-1 (punto medio)
    nodos_anterior = zeros(2,(numero_costillas*2)-1);
    nodos_larguerillos = zeros(numero_larguerillos_total,2,(numero_costillas*2)-1);
    ...
    % puntos medio
    puntos_medio_larguero_posterior= zeros(numero_costillas-1,2);
    puntos_medio_larguero_anterior = zeros(numero_costillas-1,2);
    puntos_medio_larguerillos = zeros(numero_larguerillos_total-1,2);
    

    for i = 2:numero_costillas
        puntos_medio_larguero_posterior_ala(i-1,:) = costillas(i,:,1)-costillas(i-1,:,1);
        puntos_medio_larguero_anterior_ala(i-1,:) = costillas(i,:,end)-costillas(i-1,:,end);
    end

    combined = [costillas(:,:,1); puntos_medio_larguero_posterior_ala];
    nodos_posterior = reshape(combined, 2, []);
    
    combined = [costillas(:,:,end); puntos_medio_larguero_anterior_ala];
    nodos_anterior = reshape(combined, 2, []);
    
    
    % index_larguerillos_anterior = zeros(numero_larguerillos_total-numero_larguerillos_costilla_final,1);
    % 
    % % Para guardar cuales largueros anterior que tiene que cortar los
    % % larguerillos. %Recuerda el orden de los larguerillos de arriba pa
    % % abajo
    % % size(larguerillos(i,:,end))
    % for i = numero_larguerillos_costilla_final+1:numero_larguerillos_total
    %     size(larguerillos(i,:,end))
    %     size(nodos_anterior)
    %     index_larguerillos_anterior = max(find(all(nodos_anterior < larguerillos(i,:,end)', 1)));
    % end
    ...
    larguerillo = ala.larguerillos;
    costilla  = ala.costillas;

    % Construir ribs_ribs_medio
    % ribs: Mx4 matrix [x1, y1, x2, y2] for each rib
    costilla_costilla_medio = zeros((numero_costillas * 2)-1,4); %num_costilla*2 -1 ,(x,y),(start,end)
    j=1;
    ...
    l =1 ;
    stringers1 = [larguerillo(l,:,1) larguerillo(l,:,end)]; % x_r y_r x_t y_t
    
    %larguerillo 1
    nodes =[];
    j=1;
    for j=1:numero_larguerillos_total
        stringers1 = [larguerillo(j,:,1) larguerillo(j,:,end)]; % x_r y_r x_t y_t
        for i = 1:index_larguerillos_anterior(1)-1
    
    
            ribs = costilla_costilla_medio(i,:);
            nodes = [nodes generate_stringer_nodes(stringers1, ribs)];
        end
    end
