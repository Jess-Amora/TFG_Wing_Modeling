...
function [results] = construirAla_v5(avion,datosEstructural,cargas)
...
% Parámetros de entrada:
    ...
    counter_nodos = numero_nodos_larguero_posterior_ala;
    counter_barra = numero_elementos_larguero_posterior_ala;

    for i = 1 : numero_nodos_larguero_anterior_ala-1
        barras_ala_larguero_anterior = [barras_ala_larguero_anterior [counter_nodos+i;counter_nodos+i+1]];
    end
    ...
    % for index_larguerillo_counter = 1:numero_larguerillos_costilla_final % El bucle para cada larguerillo
    % 
    %     larguerillos_i = [larguerillos(index_larguerillo_counter,:,1) larguerillos(index_larguerillo_counter,:,end)]; % x_r y_r x_t y_t
    % 
    %     for index_costilla_costilla_medio_counter = 1:index_larguerillos_anterior(index_larguerillo_counter) % El bucle para las intersecciones del larguerillo y las costillas y sus puntos medios
    %         costillas_i = costilla_costilla_medio(index_costilla_costilla_medio_counter,:);
    % 
    %         % Esta parte es para guardar las coordenadas para cada
    %         % larguerillo en la matriz.
    %         index_costilla_costilla_medio_counter
    %         index_larguerillo_counter
    %             temp = generate_stringer_nodes(larguerillos_i, costillas_i);
    % 
    %             % Take the maximum index satisfying the condition
    %             if ~isempty(temp)
    %                 matriz_nodos_larguerillos(index_costilla_costilla_medio_counter,index_larguerillo_counter,:) = temp;
    %             else
    %                 matriz_nodos_larguerillos(index_costilla_costilla_medio_counter,index_larguerillo_counter,:) = [0;0]; % No valid connection
    %             end
    % 
    %         % nodos_larguerillos = [nodos_larguerillos generate_stringer_nodes(larguerillos_i, costillas_i)];
    %         nodos_larguerillos = [nodos_larguerillos temp];
    %     end
    % end
    % 
    ...
    %     larguerillos_i = [larguerillos(index_larguerillo_counter,:,1) larguerillos(index_larguerillo_counter,:,end)]; % x_r y_r x_t y_t
    % 
    %     for index_costilla_costilla_medio_counter = 1:index_larguerillos_anterior(index_larguerillo_counter)-1 % El bucle para las intersecciones del larguerillo y las costillas y sus puntos medios
    %         costillas_i = costilla_costilla_medio(index_costilla_costilla_medio_counter,:);
    % 
    %         % Esta parte es para guardar las coordenadas para cada
    %         % larguerillo en la matriz.
    ...
    %     % entre el larguero anterior y el larguerillo
    %     [x_int,y_int] = point_slope_intersection(larguerillos(index_larguerillo_counter,1,1),larguerillos(index_larguerillo_counter,2,1), pendiente_larguero_posterior, Lf, c1*Distancia_larguero_anterior_cuerda_porcentaje, pendiente_larguero_anterior);
    %     nodos_larguerillos = [nodos_larguerillos [x_int;y_int]];
    % 
    % end
    ...
    % Se cambian las dimensiones de los nodos posterior y anterior de 2 x
    % (numero de nodos) a 1 x (numero de nodos *2)
    nodos_posterior = reshape(nodos_posterior, 1, []);
    size(nodos_posterior)
    nodos_anterior = reshape(nodos_anterior, 1, []);
