function plotAla2Dlarguerillo(avion,geom_struct, rib_struct,mesh_struct)
    ...
    b = avion.geometria.b;
    numero_costillas = rib_struct.num_costillas_total;
...
% Flag graph
    ...
    if isfield(avion.folder, 'figures') && ~isempty(avion.folder.figures)
        ...
        saveas(fig1, fullfile(avion.folder.figures, 'Step_3_larguerillos.png'));
        saveas(fig1, fullfile(avion.folder.figures, 'Step_3_larguerillos.fig'));
        disp(['📊 Saved plot to: ', fullfile(avion.folder.figures, 'Step_3_larguerillos.png')]);
    end
