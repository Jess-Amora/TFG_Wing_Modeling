function plotAla2D_mesh_solo_nodos(avion,datosEstructural,ala)
...
% Flag graph
        ...
        for i=1:2:size(nodes,2)
            if i == 1
                plot(nodes(i),nodes(i+1),'x', 'DisplayName', 'Nodos Larguerillos');
            else
                plot(nodes(i),nodes(i+1),'x', 'HandleVisibility', 'off');
            end
        end

        % Nodo posterior
        nodos_posterior = ala.mesh.nodos_posterior;
        plot(nodos_posterior(1,:),nodos_posterior(2,:),'+', 'DisplayName', 'Nodos Posterior')

        % Nodo posterior
        nodos_anterior = ala.mesh.nodos_anterior;
        plot(nodos_anterior(1,:),nodos_anterior(2,:),'*', 'DisplayName', 'Nodos Anterior')
