function plotAla2D_mesh(avion,datosEstructural,ala)
...
% Flag graph
        ...
        nodes = ala.mesh.nodos_larguerillos;

        for i=1:2:size(nodes,2)
            if i == 1
                plot(nodes(i),nodes(i+1),'xk', 'DisplayName', 'Nodos Larguerillos');
            else
                plot(nodes(i),nodes(i+1),'xk', 'HandleVisibility', 'off');
            end
        end
