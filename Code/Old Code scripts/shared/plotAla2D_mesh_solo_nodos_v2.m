function plotAla2D_mesh_solo_nodos_v2(avion,datosEstructural,ala)
...
% Flag graph
        ...
        %     if i == 1
        %         plot(nodes(i),nodes(i+1),'ok', 'DisplayName', 'Nodos Larguerillos', 'LineWidth', 0.1);
        %         % Add a label with the node number
        %         node_number = (i + 1) / 2; % Calculate node number
        %         label = sprintf('n %d', node_number); % Format the label
        %         text(nodes(i), nodes(i+1), label, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right','FontSize', 5);
        %     else
        %         plot(nodes(i),nodes(i+1),'ok', 'HandleVisibility', 'off', 'LineWidth', 0.1);
        %         % Add a label with the node number
        %         node_number = (i + 1) / 2; % Calculate node number
        %         label = sprintf('n %d', node_number); % Format the label
        %         text(nodes(i), nodes(i+1), label, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right','FontSize', 5);
