function generate_structure_v0(name)
...
fuselaje = TFG_Amora.aviones.(name).fuselaje;
...
[tri_root_stringer, quad_root_stringer, warnings] = OnlyCornerRootFront_v2(combined_nodes);
warnings = [warnings; warn(:)];
...
for index_larguerillo = 1:numero_larguerillos_total-1
...
%     [OnlyQuads; OnlyQuads_Fuselaje], [OnlyTri],[OnlyRear], plottitle, fullfile(avion.folder.figures,"OnlyPlotSurface_v4_test1_generate_structure_v14_ala14_fuselaje5_a350_1000_datos_estructual"),avion,datosEstructural);
...
write_bdf_points_v2(fullfile(avion.folder.data,'nodes.bdf'), nodes);
...
write_bdf_lines_v3(fullfile(avion.folder.data,'\lines_v5.bdf'), lines_updated, material_info, property_info);
...
write_bdf_quads_v1(fullfile(avion.folder.data,'\quads_all_v1.bdf'), quads_all_3D_processed, [], pshell_info, material_info);
...
write_bdf_tris_v1(fullfile(avion.folder.data,'\tri_v1.bdf'), tri_all_3D_processed, pshell_info, material_info);
...
generate_boundary_conditions_bdf(fullfile(avion.folder.data,'\BC_v1.bdf'), root_nodes, root_front_spar_intrados, root_rear_spar_intrados, rib_fuselage_nodes);
