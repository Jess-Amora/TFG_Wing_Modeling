...
%   triangles - Table containing 'local_id', 'node_1', 'node_2', 'node_3', 'tag'.
%
% Output:
...
%                      [Triangle ID, Property ID, G1, G2, G3, THETA, ZOFFS].
    ...
    required_vars = {'local_id', 'node_1', 'node_2', 'node_3', 'tag'};
...
function [G1, G2, G3] = find_global_tri_ids(current_tri, combined_nodes_3D)
    ...
    %   G1, G2, G3, G4 - Global IDs for the 4 nodes of the quad.
        ...
        % Analyze
    [num_stringers_last_rib, max_rib_index, max_stringer_index, rib_ranges, rib_ranges_by_ribs, special_rib_indices, max_ribs_fuselaje] = analyze_stringer_rib_data(combined_nodes_3D);
    

    switch current_tri.tag
        ...
        case "tri last front"
            ...
            node_result3 = combined_nodes_3D(combined_nodes_3D.stringer_index == -1 & ...
                                             ...
                                             combined_nodes_3D.h == current_tri.h, :);
        otherwise
            error('Unknown tag: %s', current_tri.tag);
    end
    ...
    if height(node_result1) == 1 && height(node_result2) == 1 && ...
       height(node_result3) == 1
        ...
        node_result2
        node_result3
        warning('Ambiguity or missing nodes for quad tag %s. Skipping this quad.', current_tri.tag);
