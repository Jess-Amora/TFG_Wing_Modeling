function [quad_surfaces, warnings] = create_surfaces_vertical_ribs_wing(combined_nodes_3D)
% CREATE_SURFACES_VERTICAL_RIBS_WING - Generates quadrilateral surfaces for vertical ribs in the wing.
%
% Inputs:
...
%   warnings - Cell array containing warnings for skipped or invalid surfaces.
    ...
    front_spar_extrados = extract_sorted_nodes_3D(combined_nodes_3D, 'front spars', 'extrados');
    front_spar_intrados = extract_sorted_nodes_3D(combined_nodes_3D, 'front spars', 'intrados');
    rear_spar_extrados = extract_sorted_nodes_3D(combined_nodes_3D, 'rear spars', 'extrados');
    rear_spar_intrados = extract_sorted_nodes_3D(combined_nodes_3D, 'rear spars', 'intrados');
    ...
    for i = 2:2:num_ribs
        ...
        quad_surfaces = append_quad_surface_3D(quad_surfaces, nodes, surface_counter, "quad vertical rib", area, aspect_ratio);
