function [quad_surfaces, warnings] = create_surfaces_vertical_front_spar_fuselaje(combined_nodes_3D)
% CREATE_SURFACES_VERTICAL_FRONT_SPAR_FUSELAJE - Generates quadrilateral surfaces for the vertical front spar in the fuselage.
%
% Inputs:
...
%   warnings - Cell array containing warnings for skipped or invalid surfaces.
    ...
    front_spar_extrados = extract_nodes_3D(combined_nodes_3D, 'front spars fuselaje', 'extrados');
    front_spar_intrados = extract_nodes_3D(combined_nodes_3D, 'front spars fuselaje', 'intrados');

    if isempty(front_spar_extrados) || isempty(front_spar_intrados)
        warnings{end+1} = '⚠️ No valid nodes found for the front spar fuselage.';
        return;
    end
    ...
    if num_ribs < 1
        warnings{end+1} = '⚠️ Insufficient nodes for front spar fuselage surface creation.';
        return;
    end
    ...
    for i = 1:num_ribs - 1
        ...
        quad_surfaces = append_quad_surface_3D(quad_surfaces, nodes, surface_counter, "quad vertical front fuselaje", area, aspect_ratio);
        surface_counter = surface_counter + 1;
    end

    %% 🔗 Connect Front Spar to Root (Encastre)
    nodes = [
        front_spar_extrados(num_ribs, :); % Bottom-left
        front_spar_intrados(num_ribs, :); % Top-left
        extract_nodes_3D(combined_nodes_3D, 'front spars', 'extrados', 1e5); % Top-right
        extract_nodes_3D(combined_nodes_3D, 'front spars', 'intrados', 1e5)  % Bottom-right
    ];
    ...
    quad_surfaces = append_quad_surface_3D(quad_surfaces, nodes, surface_counter, "quad vertical front fuselaje root", area, aspect_ratio);
end
