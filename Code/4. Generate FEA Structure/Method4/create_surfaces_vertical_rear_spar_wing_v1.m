function [quad_surfaces, warnings] = create_surfaces_vertical_rear_spar_wing_v1(combined_nodes_3D)
    ...
    for i = 1:min(height(rear_spar_extrados), height(rear_spar_intrados)) - 1 
        ...
        node1 = rear_spar_extrados(i, :);          % Bottom-left
        node2 = rear_spar_intrados(i, :)   ;          % Top-left
        node3 = rear_spar_intrados(i + 1, :);         % Top-right
        node4 = rear_spar_extrados(i + 1, :);      % Bottom-right
