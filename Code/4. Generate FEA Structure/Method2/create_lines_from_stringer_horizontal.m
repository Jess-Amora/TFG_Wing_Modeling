function [lines, line_counter] = create_lines_from_stringer_horizontal(vector_stringer, stringer_index, tag, h, initial_line_counter)
...
%   line_counter      - Updated line counter after adding all lines.
    ...
    for i = 1:(height(vector_stringer) - 1)
        % Extract consecutive nodes
        node_1 = vector_stringer(i, :);
        node_2 = vector_stringer(i + 1, :);

        % Extract rib indices from the nodes
        rib_1 = node_1.rib_index;
        rib_2 = node_2.rib_index;

        % Use the create_new_line function to create a line
        [lines, line_counter] = create_new_line_horizontal(node_1, node_2, stringer_index, rib_1, rib_2, tag, h, line_counter, lines);
    end
end
