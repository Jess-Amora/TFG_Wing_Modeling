...
for i = 1:num_ribs - 1
    ...
    node1 = combined_nodes_fuselaje(combined_nodes_fuselaje.rib_index == i & combined_nodes_fuselaje.tag == 'rear spars fuselaje', :);          % Bottom-left
    node2 = current_stringer_nodes(i, :);        % Top-left
    node3 = current_stringer_nodes(i + 1, :);         % Top-right
    node4 = combined_nodes_fuselaje(combined_nodes_fuselaje.rib_index == i + 1 & combined_nodes_fuselaje.tag == 'rear spars fuselaje', :);          % Bottom-right
...
% Extract nodes for the quadrilateral
node1 = combined_nodes_fuselaje(combined_nodes_fuselaje.rib_index == num_ribs & combined_nodes_fuselaje.tag == 'rear spars fuselaje', :);          % Bottom-left
node2 = current_stringer_nodes(num_ribs, :);        % Top-left
node3 = combined_nodes(combined_nodes.rib_index == -1 & combined_nodes.stringer_index == 1, :);          % Top-right
node4 = combined_nodes(combined_nodes.rib_index == start_rib & combined_nodes.tag == 'rear spars', :);          % Bottom-right
