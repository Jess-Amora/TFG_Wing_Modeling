function [quad_surfaces, warnings] = create_surfaces_stringer_fuselaje_v1(combined_nodes_fuselaje, combined_nodes, stringer_index)
    ...
    for i = 1:num_ribs - 1
        ...
        node2 = next_stringer_nodes(i, :)   ;          % Top-left
        node3 = next_stringer_nodes(i + 1, :);         % Top-right
        node4 = current_stringer_nodes(i + 1, :);      % Bottom-right
...
node1 = current_stringer_nodes(i, :);          % Bottom-left
node2 = next_stringer_nodes(i, :)   ;          % Top-left
node3 = combined_nodes(combined_nodes.rib_index == -1 & combined_nodes.stringer_index == stringer_index + 1, :);         % Top-right
node4 = combined_nodes(combined_nodes.rib_index == -1 & combined_nodes.stringer_index == stringer_index, :);          % Top-right      % Bottom-right
