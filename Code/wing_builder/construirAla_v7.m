...
function [results] = construirAla_v7(avion,datosEstructural,cargas)
...
% Parámetros de entrada:
    ...
    Numero_nodos_elementos = zeros(2 + numero_larguerillos_total,2);
    ...
    Numero_nodos_elementos(1,1) = numero_nodos_larguero_posterior_ala;
    Numero_nodos_elementos(2,1) = numero_nodos_larguero_anterior_ala;
    ...
    for index_larguerillo_counter = 1:numero_larguerillos_costilla_final % El bucle para cada larguerillo
        ...
        temp_size_nodos = size(nodos_larguerillos)
        % Hay que unir el primer nodo del larguerillo con el encastre
        % (línea vertical). 
        temp_intersect = cortes_de_dos_funciones_lineales([Lf 1],inf,[intersecciones_costillas_larguerillos(index_larguerillo_counter,index_counter_quitar_nodos_larguerillos_menor_Lf(index_larguerillo_counter)+1,:)], pendiente_larguero_posterior)
    ...
    string = strcat('mesh\v',num2str(7),'_points.csv');
    ...
    string = strcat('mesh\v',num2str(7),'_elements.csv');
