function [results] = generate_wing_v4(avion,datosEstructural,cargas,databasePath)
...
% Parámetros de entrada:
    ...
    coord_costillas_larguero_posterior_y_triangulo
    coord_costillas_larguero_posterior_x_triangulo
    Lf
    ...
    % assignin('base', 'coord_aerodinamica_costillas_triangulo', coord_aerodinamica_costillas_triangulo);
    % assignin('base', 'coord_aerodinamica_costillas_punto_medio_triangulo', coord_aerodinamica_costillas_punto_medio_triangulo);
    ...
    assignin('base', 'coord_aerodinamica_costillas_punto_medio', coord_aerodinamica_costillas_punto_medio);
    assignin('base', 'coord_aerodinamico_costillas', coord_aerodinamico_costillas);
    ...
    %%
    assignin('base', 'costillas', costillas);
    ...
    for index_larguerillo_counter = 1:numero_larguerillos_costilla_final % El bucle para cada larguerillo
        ...
        max_index = size(intersecciones_costillas_larguerillos, 2);
valid_index = min(index_larguerillos_anterior(index_larguerillo_counter), max_index);

if valid_index < index_counter_quitar_nodos_larguerillos_menor_Lf(index_larguerillo_counter) + 1
    warning('Invalid indexing detected. Adjusting to within bounds.');
    valid_index = index_counter_quitar_nodos_larguerillos_menor_Lf(index_larguerillo_counter) + 1;
end

nodos_larguerillos = [nodos_larguerillos temp_intersect intersecciones_costillas_larguerillos(index_larguerillo_counter, ...
    index_counter_quitar_nodos_larguerillos_menor_Lf(index_larguerillo_counter)+1:valid_index,:)];
    ...
    databasePath = fullfile(databasePath,'plot_2D_ala');
    plotAla2D_mesh_solo_nodos_v6(avion,datosEstructural,results,plottitle,'' ,'',databasePath);
