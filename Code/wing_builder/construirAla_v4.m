...
function [results] = construirAla_v4(avion,datosEstructural,cargas)
...
% Parámetros de entrada:
    ...
    % puntos medio
    puntos_medio_larguero_posterior= zeros(numero_costillas-1,3);
    puntos_medio_larguero_anterior = zeros(numero_costillas-1,3);
    puntos_medio_larguerillos = zeros(numero_larguerillos_total-1,3);
    ...
    j=1;
    for j=1:numero_larguerillos_costilla_final % El bucle para cada larguerillo

        larguerillos_i = [larguerillos(j,:,1) larguerillos(j,:,end)]; % x_r y_r x_t y_t

        for i = 1:index_larguerillos_anterior(j) % El bucle para las intersecciones del larguerillo y las costillas y sus puntos medios
            costillas_i = costilla_costilla_medio(i,:);
            nodes_larguerillos = [nodes_larguerillos generate_stringer_nodes(larguerillos_i, costillas_i)];
        end
    end
    
    for j=numero_larguerillos_costilla_final+1:numero_larguerillos_total % El bucle para cada larguerillo
        ...
        [x_int,y_int] = point_slope_intersection(larguerillos(j,1,1), larguerillos(j,2,1), pendiente_larguero_posterior, Lf, c1*Distancia_larguero_anterior_cuerda_porcentaje, pendiente_larguero_anterior);
        nodes_larguerillos = [nodes_larguerillos x_int y_int];
    ...
    results.mesh.nodes_larguerillos = nodes_larguerillos; % larguerillos
