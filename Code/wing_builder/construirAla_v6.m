...
function [results] = construirAla_v6(avion,datosEstructural,cargas)
...
% Parámetros de entrada:
    ...
    numero_larguerillos_costilla_final = floor(norm(costillas(end,:,1)-costillas(end,:,end))/distancia_entre_larguerillo);
    ...
    % Creando los nodos en los largueros posterior y anterior
    ...
    fprintf('Numero de nodos en el larguero posterior = %d\n', numero_nodos_larguero_posterior_ala);
    fprintf('Nodos 1 : %d\n', numero_nodos_larguero_posterior_ala);
    fprintf('Numero de nodos en el larguero anterior = %d\n', numero_nodos_larguero_anterior_ala);
    fprintf('Nodos %d : %d\n', numero_nodos_larguero_posterior_ala+1,numero_nodos_larguero_posterior_ala+numero_nodos_larguero_anterior_ala);

    % % Buscando los número de cortes que corten los larguerillos incluyendo
    % el punto medio entre costillas
    ...
    % Matriz de las coordenadas de las intersecciones de los
    ...
    for index_larguerillo_counter = 1:numero_larguerillos_costilla_final % El bucle para cada larguerillo
        ...
        nodos_larguerillos = [nodos_larguerillos intersecciones_costillas_larguerillos(index_larguerillo_counter,1:index_larguerillos_anterior(index_larguerillo_counter),:)];

        counter_nodo_larguerillo_ala = counter_nodo_larguerillo_ala + index_larguerillos_anterior(index_larguerillo_counter);
        fprintf('Numero de nodos en el larguerillo numero %d = %d\n',index_larguerillo_counter, index_larguerillos_anterior(index_larguerillo_counter));
        fprintf('Nodos %d : %d\n', counter_nodo_larguerillo_ala - index_larguerillos_anterior(index_larguerillo_counter)+1,counter_nodo_larguerillo_ala);
    end
    ...
    for index_larguerillo_counter = numero_larguerillos_costilla_final+1:numero_larguerillos_total % El bucle para cada larguerillo
        ...
        if index_larguerillos_anterior(index_larguerillo_counter)>1
            nodos_larguerillos = [nodos_larguerillos intersecciones_costillas_larguerillos(index_larguerillo_counter,1:index_larguerillos_anterior(index_larguerillo_counter)-1,:)];
    ...
    barras_ala_larguero_anterior = [];
    barras_ala_larguerillos = [];


    %% Creando los primeros elementos en el larguero posterior
    for i = 1 : numero_nodos_larguero_posterior_ala-1
        barras_ala_larguero_posterior = [barras_ala_larguero_posterior [i;i+1]];
    end
    numero_elementos_larguero_posterior_ala = size(barras_ala_larguero_posterior,2);
    fprintf('Numero de elementos en el larguero posterior = %d\n', numero_elementos_larguero_posterior_ala);
    fprintf('Elementos 1 : %d\n', numero_elementos_larguero_posterior_ala);
    
    counter_nodos_larguero_posterior_ala = numero_nodos_larguero_posterior_ala;

    %% Creando Los elementos en el larguero anterior
    for i = 1 : numero_nodos_larguero_anterior_ala-1
        barras_ala_larguero_anterior = [barras_ala_larguero_anterior [counter_nodos_larguero_posterior_ala+i;counter_nodos_larguero_posterior_ala+i+1]];
    end
    numero_elementos_larguero_anterior_ala = size(barras_ala_larguero_anterior,2);
    fprintf('Numero de elementos en el larguero anterior = %d\n', numero_elementos_larguero_anterior_ala);
    fprintf('Elementos %d : %d\n', numero_elementos_larguero_posterior_ala+1,numero_elementos_larguero_posterior_ala+numero_elementos_larguero_anterior_ala);

    % Es el número de barra en donde empieza los elementos de los
    % larguerillos;
    counter_barras = numero_elementos_larguero_posterior_ala+numero_elementos_larguero_anterior_ala;
    % Está counter es donde empezó el nodo de los larguerillos.
    counter_barras_nodos = numero_nodos_larguero_posterior_ala+numero_nodos_larguero_anterior_ala;
    counter_barras_nodos
    ...
    for index_larguerillo_counter = 1:numero_larguerillos_total % El bucle para cada larguerillo
        counter_barra_minus_i = counter_barras; % Es el contador de barra del larguerillo anterior.
        counter_barras_nodos_minus_i = counter_barras_nodos + 1 ; % Es el contador del nodo del larguerillo anterior.
        
        % Construyendo la barra en larguerillo numero = index_larguerillo_counter  
        for index_nodos = 1:index_larguerillos_anterior(index_larguerillo_counter)-1
            % counter_barras_nodos_minus_i + index_nodos
            % counter_barras_nodos_minus_i + index_nodos + 1
            barras_ala_larguerillos = [barras_ala_larguerillos [counter_barras_nodos_minus_i + index_nodos;counter_barras_nodos_minus_i + index_nodos + 1]];
            counter_barras = counter_barras + 1;
        end
        ...
        fprintf('---Numero de barras en el larguerillo numero %d = %d\n',index_larguerillo_counter, numeros_elementos_larguerillos_index);
        fprintf('Elements %d : %d\n', counter_barra_minus_i+1,counter_barras);
        fprintf('nodos %d : %d\n', counter_barras_nodos_minus_i,counter_barras_nodos);
    end
    ...
    nodos = nodos_ala_global;
    % size(nodos)
    
    % Rehacer los nodos 2D -> 3D 
    % Reshape nodos_ala_global to separate x and y
    num_nodes = size(nodos_ala_global,1); % Number of nodes
    nodos_matrix = reshape(nodos_ala_global, 2, num_nodes); % Reshape to 2x(number of nodes)
        
    % Add z = 0
    z_row = zeros(1, num_nodes); % Create a row of zeros for z-coordinates
    nodos = [nodos_matrix; z_row]'; % Combine x, y, and z
    % size(nodos)
    % % Flatten back to 1x(3 * number of nodes)
    % nodos = reshape(nodos_matrix_3D, 1, []); % 1x(3 * number of nodes)
    % nodos
    % Save points to a .csv file with fixed decimal formatting

    % size(nodos)
    points_table = array2table(nodos, 'VariableNames', {'x', 'y', 'z'});
    string = strcat('mesh\v',num2str(6),'_points.csv');
    writetable(points_table, string, 'WriteVariableNames', true, ...
        'FileType', 'text');

    % size(barras_ala_global)
    % Convert elements to floating-point format
    elements_table = array2table(barras_ala_global, 'VariableNames', {'Point1', 'Point2'});
    
    % Write elements to a .csv file with custom formatting
    string = strcat('mesh\v',num2str(6),'_elements.csv');
    writetable(elements_table, string, 'WriteVariableNames', true, ...
    'FileType', 'text');
