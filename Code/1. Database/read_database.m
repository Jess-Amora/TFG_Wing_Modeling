function read_database(databasePath)
    % ===========================================================
    % 📌 Function: read_database (UPDATED)
    % ===========================================================
    % Reads structural, aircraft, and material data from CSV files
    % and loads them into the `TFG_Amora.mat` database.
    %
    % ✅ Reads:
    %   - `structural_parameters.csv` → datosEstructural
    %   - `aircraft_data.csv` → aircraft_data
    %   - `materials.csv` → materials (NEW)
    %
    % ✅ Saves:
    %   - `TFG_Amora.datosEstructural`
    %   - `TFG_Amora.aircraft_data`
    %   - `TFG_Amora.materials` (NEW)
    %
    % ===========================================================
    ...
    materialFile = fullfile(databasePath, 'Data', 'materials.csv'); % NEW
    ...
    if isfile(tfgFile)
        ...
        if ~isfield(TFG_Amora, 'materials')
            warning('⚠️ `materials` was missing. Creating an empty struct.');
            TFG_Amora.materials = struct();
        end
    else
    ...
    % ✅ Step 4: Read Material Properties CSV (NEW)
    if isfile(materialFile)
        materialData = readtable(materialFile, 'PreserveVariableNames', true);
        for i = 1:height(materialData)
            name = materialData.name{i};
            materialStruct = struct(...
                'E', materialData.E(i), ...
                'nu', materialData.nu(i), ...
                'rho', materialData.rho(i), ...
                'sigma_lim', materialData.sigma_lim(i), ...
                'sigma_rot', materialData.sigma_rot(i), ...
                'sigma_cort', materialData.sigma_cort(i), ...
                'type', materialData.type_material(i) ...
            );

            % Save material in TFG_Amora.materials.material_(name)
            materialField = ['material_', name]; % e.g., material_Aluminum_7075_T6
            TFG_Amora.materials.(materialField) = materialStruct;
        end
        disp('✅ Material properties loaded successfully.');
    else
        warning('⚠️ No materials.csv file found.');
    end
