function updated_node_vector = insert_perpendicular_node_v4(node_vector, slope, point, ribs_total)
% Updated Function to insert a node into a 1xNx4 node vector based on geometry and slope.
% Rib and Stringer indices are inferred dynamically.
...
%   updated_node_vector: 1x(N+1)x4 matrix with the new node inserted in order
    ...
    %% Infer rib_index and stringer_index
    if insert_idx > 0
        % Infer rib index dynamically
        rib_index = node_vector(1, insert_idx, 3) + 1; % Increment rib index logically
        stringer_index = node_vector(1, insert_idx, 4);
    else
        % Use the last node's rib and stringer indices as fallback
        rib_index = node_vector(1, end, 3) + 1; % Increment rib index logically
