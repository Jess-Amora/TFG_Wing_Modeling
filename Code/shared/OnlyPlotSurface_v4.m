function OnlyPlotSurface_v4(combined_nodes,combined_nodes_fuselaje, inserted_table, quad_surfaces, tri_surfaces, penta_surfaces, rear_surfaces, plottitle, plotfilename,avion,datosEstructural)
    ...
    if ~isempty(quad_surfaces)
        for i = 1:height(quad_surfaces)
            ...
            elseif strcmp(surface_tag, "quad fuselaje front")
                ...
                % node_1 = current_stringer_nodes(i, :);        % Top-left
                % node_2 = combined_nodes_fuselaje(combined_nodes_fuselaje.rib_index == i & combined_nodes_fuselaje.tag == 'front spars fuselaje', :);          % Bottom-left
                % node_3 = combined_nodes_fuselaje(combined_nodes_fuselaje.rib_index == i + 1 & combined_nodes_fuselaje.tag == 'front spars fuselaje', :);          % Bottom-right
                % node_4 = current_stringer_nodes(i + 1, :);         % Top-right
