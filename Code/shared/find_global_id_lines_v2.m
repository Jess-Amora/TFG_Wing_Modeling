function lines_updated = find_global_id_lines(combined_nodes_3D_processed, lines)
    ...
    %
    % Outputs:
    %   lines_updated - Updated lines table with new columns:
    ...
    if ~all(ismember({'tag', 'rib_1', 'rib_2', 'stringer_index'}, lines.Properties.VariableNames))
        error('The lines table must contain "tag", "rib_1", "rib_2", and "stringer_index" columns.');
    end

    % Step 2: Initialize the updated lines table
    lines.global_id_Point1 = NaN(height(lines), 1); % Preallocate
    lines.global_id_Point2 = NaN(height(lines), 1); % Preallocate
        ...
        switch current_tag
            case "barra costilla"
                ...
                for j = 1:height(tag_lines)
                    ...
                    lines.global_id_Point1(idx) = Point1;
                    lines.global_id_Point2(idx) = Point2;
                end
