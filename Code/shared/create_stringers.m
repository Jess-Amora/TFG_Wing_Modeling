function [horizontal_stringers,vertical_stringers] = create_stringers(combined_nodes_3D)
...
for index_larguerillo = 1:max_stringer_index
    ...
    larguerillo_wing = combined_nodes_3D(combined_nodes_3D.tag=='stringer' & combined_nodes_3D.h == 'extrados' & combined_nodes_3D.rib_index >= rib_ranges(index_larguerillo,2),:);

    larguerillo = [larguerillo_fuselage(1:end-1,:); encastre; larguerillo_wing];


    [larguerillo_i, line_counter] = create_lines_from_stringer_horizontal(larguerillo, index_larguerillo, "stringer", "extrados", 1);
