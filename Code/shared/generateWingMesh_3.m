function mesh = generateWingMesh_3(geom_struct, rib_struct, avion)
...
%
    ...
    distancia_entre_larguerillo_vertical = datosEstructural.distancia_entre_larguerillo/cos(geom_struct.alfa_larg_posterior);
    alfa_larguero_posterior_radianes = geom_struct.alfa_larg_posterior;
    ...
    nodos_larguerillos =[];
    id_nodo_local_larguerillo_costilla = zeros(numero_larguerillos_total*num_costillas,3);
    ...
    for index_larguerillo_counter = 1:numero_larguerillos_costilla_final % El bucle para cada larguerillo
        ...
        % (línea vertical). 
        temp_intersect = cortes_de_dos_funciones_lineales_v3([Lf 1],inf,[intersecciones_costillas_larguerillos(index_larguerillo_counter,index_counter_quitar_nodos_larguerillos_menor_Lf(index_larguerillo_counter)+1,:)], pendiente_post,index_larguerillo_counter,-1);
    ...
    for index_larguerillo_counter = numero_larguerillos_costilla_final+1:numero_larguerillos_total % El bucle para cada larguerillo
        ...
        if index_larguerillos_anterior(index_larguerillo_counter)>1
            ...
            temp = cortes_de_dos_funciones_lineales_v3(larguerillos(index_larguerillo_counter,:,1), pendiente_post, [Lf c1*Dist_ant], pendiente_ant,index_larguerillo_counter,-2);
            nodos_larguerillos = [nodos_larguerillos temp];
            threshold_distance = distancia_entre_costillas*.07;
            
            % nodos_larguerillos = insert_perpendicular_node_v3(nodos_larguerillos, pendiente_post, [x2 y2], num_costillas*2-1,threshold_distance);
            slope = pendiente_post;
            perpendicular_slope = pendiente_perp;    
            
            [nodos_larguerillos inserted_nodes_temp]= adjust_nodos_larguerillos_v2(nodos_larguerillos, slope, perpendicular_slope, num_costillas*2-1, distancia_entre_larguerillo_vertical, alfa_larguero_posterior_radianes, threshold_distance);
    ...
    mesh.index_counter_quitar_nodos_larguerillos_menor_Lf = index_counter_quitar_nodos_larguerillos_menor_Lf;
    mesh.index_larguerillos_anterior = index_larguerillos_anterior;
    % mesh.nodes_inside_limit = nodes_inside_limit;
    % mesh.all_inside_nodes = all_inside_nodes;
    ...
    mesh.nodos_larguerillos = nodos_larguerillos;
