function [quad_surfaces, warnings] = create_surfaces_vertical_front_spar_fuselaje_v1(combined_nodes_3D)
...
%% Unir con los puntos en el encastre
...
%          combined_nodes_3D.h == 'intrados', :);         
node_3 = combined_nodes_3D(combined_nodes_3D.tag == 'front spars' & ...
         combined_nodes_3D.h == 'extrados' & combined_nodes_3D.rib_index == 1e5, :);         
node_4 = combined_nodes_3D(combined_nodes_3D.tag == 'front spars' & ...
         combined_nodes_3D.h == 'intrados' & combined_nodes_3D.rib_index == 1e5, :);     
% Ensure nodes are not empty
if isempty(node_1) || isempty(node_2) || isempty(node_3) || isempty(node_4)
    warnings{end+1} = sprintf('Skipping surface at rib %d due to missing nodes.', start_rib + i - 1);
    % continue;
