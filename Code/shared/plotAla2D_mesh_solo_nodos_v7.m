function plotAla2D_mesh_solo_nodos_v7(avion,geom_struct,ribs_struct,mesh_struct)
...
% Flag graph
        ...
        % Ala Costillas
        % numero_costillas = numero_costillas;
        numero_costillas =  ribs_struct.num_costillas_total;
        ...
        %Ala larguerillo
        numero_larguerillo_total = size(mesh_struct.larguerillos,1);
        larguerillos = mesh_struct.larguerillos;
        ...
        % Nodo posterior
        nodos_posterior = mesh_struct.nodos_posterior';
        ...
        % Nodo anterior
        nodos_anterior = mesh_struct.nodos_anterior';
        % size(nodos_anterior)
        ...
        % size(mesh.nodos_larguerillos)
        % nodes = mesh_struct.nodos_intersecciones;
        nodes = mesh_struct.nodos_larguerillos;
        % size(nodes)
        % counter_nodes
        ...
        % % Nodos en los larguerillos
        % % size(mesh.nodos_larguerillos)
        % nodes = squeeze(mesh.larguerillos)';
        % % size(nodes)
        % % counter_nodes
        % for i=1:size(nodes,1)
        %     if i == 1
    ...
    if isfield(avion.folder, 'figures') && ~isempty(avion.folder.figures)
        ...
        saveas(fig, fullfile(avion.folder.figures, 'Step_3_nodos_larguerillos.png'));
        saveas(fig, fullfile(avion.folder.figures, 'Step_3_nodos_larguerillos.fig'));
        disp(['📊 Saved plot to: ', fullfile(avion.folder.figures, 'Step_3_nodos_larguerillos.png')]);
    end
