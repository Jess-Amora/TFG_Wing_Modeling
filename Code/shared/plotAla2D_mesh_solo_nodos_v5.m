function plotAla2D_mesh_solo_nodos_v5(avion, datosEstructural, ala, plotTitle, xAxisLabel, yAxisLabel, savePath)
...
% Flag graph
    ...
    % Add titles and labels if provided
    ...
    legend('Geometry Trace', 'Leading Edge', 'Trailing Edge', 'Location', 'best');
    hold off;
    ...
    if exist('savePath', 'var') && ~isempty(savePath)
        ...
        if isempty(ext)
            % Save as both PNG and MATLAB FIG file if no extension provided
            saveas(fig, fullfile(filePath, [fileName, '.png']));
            savefig(fig, fullfile(filePath, [fileName, '.fig']));
        elseif strcmp(ext, '.fig')
            % Save specifically as a MATLAB FIG file
            savefig(fig, savePath);
        else
            % Save using the specified extension
            saveas(fig, savePath);
            % Also save a .fig version for editing later
            savefig(fig, fullfile(filePath, [fileName, '.fig']));
        end
    ...
    % Close the figure after saving (optional, for automated scripts)
    close(fig);
end
