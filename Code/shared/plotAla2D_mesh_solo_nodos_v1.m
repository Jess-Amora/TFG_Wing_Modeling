function plotAla2D_mesh_solo_nodos_v1(avion,datosEstructural,ala)
...
% Flag graph
        ...
        for i=1:2:size(nodos_posterior,2)
            if i == 1
                ...
                text(nodos_posterior(i), nodos_posterior(i+1), label, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right','FontSize', 5);
            else
        ...
        nodes = ala.mesh.nodes_larguerillos;

        % for i=1:2:size(nodes,2)
        %     if i == 1
        %         plot(nodes(i),nodes(i+1),'xk', 'DisplayName', 'Nodos Larguerillos', 'LineWidth', 0.1);
        %         % Add a label with the node number
        %         node_number = (i + 1) / 2; % Calculate node number
        %         label = sprintf('n %d', node_number); % Format the label
        %         text(nodes(i), nodes(i+1), label, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right','FontSize', 5);
        %     else
        %         plot(nodes(i),nodes(i+1),'xk', 'HandleVisibility', 'off', 'LineWidth', 0.1);
        %         % Add a label with the node number
        %         node_number = (i + 1) / 2; % Calculate node number
        %         label = sprintf('n %d', node_number); % Format the label
        %         text(nodes(i), nodes(i+1), label, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right','FontSize', 5);
        ...
        % plot(nodos_posterior(1,:),nodos_posterior(2,:),'+y', 'DisplayName', 'Nodos Posterior')
        % 
        % % Nodo anterior
        % nodos_anterior = ala.mesh.nodos_anterior;
        % plot(nodos_anterior(1,:),nodos_anterior(2,:),'*g', 'DisplayName', 'Nodos Anterior')
