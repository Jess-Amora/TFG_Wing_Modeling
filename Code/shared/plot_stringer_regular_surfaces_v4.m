function plot_stringer_regular_surfaces_v4(combined_nodes, quad_surfaces_regular,avion )
...
% Flag graph
    ...
    axis equal;
    grid on;
    title( 'Plot 2D quad regular'); % Use the custom title from input
    xlabel('X Coordinate');
    ylabel('Y Coordinate');
    ...
    stringers_idx = strcmp(combined_nodes.tag, 'stringer');  % Logical index for stringers nodes
    ...
    if isfield(folder, 'figures') && ~isempty(folder.figures)
        ...
        saveas(fig, fullfile(folder.figures, 'Step_4_regular_quad_2D.png'));
        saveas(fig, fullfile(folder.figures, 'Step_4_regular_quad_2D.fig'));
        disp(['📊 Saved plot to: ', fullfile(folder.figures, 'Step_4_regular_quad_2D.png')]);
    end
