function superficie_horizontal_larguero_pasterior = create_rear_spar_surfaces_v3(...
...
%
% Outputs:
%   superficie_horizontal_larguero_pasterior: Table with columns:
%       - surface_id: Unique surface identifier
%       - TEMP_ID_1: Temporary ID for first rib
%       - TEMP_ID_2: Temporary ID for second rib
%       - Node_ID_1: Local node ID for first node
%       - Node_ID_2: Local node ID for second node
%       - rib_start: Starting rib index
%       - rib_end: Ending rib index
%       - stringer_start: Starting stringer index
%       - stringer_end: Ending stringer index
%       - tag: Surface type (e.g., 'rear_spar_surface')
    ...
    %% 📝 Initialize Output
    superficie_horizontal_larguero_pasterior = table([], [], [], [], [], [], [], [], [], [], 'VariableNames', ...
        {'surface_id', 'TEMP_ID_1', 'TEMP_ID_2', 'Node_ID_1', 'Node_ID_2', ...
        'rib_start', 'rib_end', 'stringer_start', 'stringer_end', 'tag'});
    ...
    for index_costilla = start_rib:end_rib
        ...
        % Filter nodes by rib and stringer
        ...
        new_surface = table( ...
            surface_counter, ...                          % surface_id
            TEMP_ID_BASE_larguero_posterior - target_ribs(1), ... % TEMP_ID_1
            TEMP_ID_BASE_larguero_posterior - target_ribs(2), ... % TEMP_ID_2
            node_id_rib1, ...                             % Node_ID_1
            node_id_rib2, ...                             % Node_ID_2
            target_ribs(1), ...                           % rib_start
            target_ribs(2), ...                           % rib_end
            target_stringers, ...                         % stringer_start
            target_stringers, ...                         % stringer_end
            "rear_spar_surface", ...                      % tag
            'VariableNames', {'surface_id', 'TEMP_ID_1', 'TEMP_ID_2', 'Node_ID_1', ...
                              'Node_ID_2', 'rib_start', 'rib_end', 'stringer_start', ...
                              'stringer_end', 'tag'});
        
        superficie_horizontal_larguero_pasterior = [superficie_horizontal_larguero_pasterior; new_surface];
