function plotAla2Dcostilla_total(avion,datosEstructural,ala,fuselaje)
...
% Flag graph
        ...
        % Cajón fuselaje (puntos rallados)
        ...
        costillas_total = [costillas_fuselaje;costillas];

        for i=1:numero_costillas+numero_costillas_fuselaje
            if i == 1
                plot(squeeze(costillas_total(i,1,:)),squeeze(costillas_total(i,2,:)),'k', 'DisplayName', 'Costillas');
            else
                plot(squeeze(costillas_total(i,1,:)),squeeze(costillas_total(i,2,:)),'k', 'HandleVisibility', 'off');
            end
        end
