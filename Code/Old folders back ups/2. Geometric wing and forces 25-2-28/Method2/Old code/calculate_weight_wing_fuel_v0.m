function results = calculate_final_forces_v1(ala, avion, cargas, rib_prop)
...
%% Now compute Wing & Fuel weight from chord
wing_weight_total = 0.1 * MTOW;      % 10% of total weight
fuel_weight_total = 0.2 * MTOW;      % 20% of total weight

% Extract chord array from rib_props
chord_all = rib_props.C;  % e.g. length = #ribs

sumC3 = sum(chord_all.^3);
k_ala = wing_weight_total / sumC3;
k_comb = fuel_weight_total / sumC3;

% Distribute with load factor included
W_ala_full = k_ala * chord_all.^3 * n;
W_comb_full = k_comb * chord_all.^3 * n;

fprintf('Check Wing Weight:  sum(W_ala_full) = %.2f N, expected ~ %.2f\n',...
    sum(W_ala_full), n*wing_weight_total);
fprintf('Check Fuel Weight:  sum(W_comb_full)= %.2f N, expected ~ %.2f\n',...
    sum(W_comb_full), n*fuel_weight_total);

% If you only want internal stations (like i=2..end-1),
% you can define a subrange:
