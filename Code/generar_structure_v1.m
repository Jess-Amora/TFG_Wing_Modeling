...
% Ensure it's properly reshaped into a 2D matrix (rows × columns)
nodos_larguerillos = reshape(permute(nodos_larguerillos, [3, 2, 1]), [], num_coords);  % (num_ribs * num_stringers) × 2

% Generate correct rib and stringer indices
rib_indices = repelem((1:num_ribs)', num_stringers);    % Repeat rib index for each stringer
stringer_indices = repmat((1:num_stringers)', num_ribs, 1);  % Repeat stringer index for each rib

% Concatenate data
nodos_larguerillos = [nodos_larguerillos, rib_indices, stringer_indices];

% Convert to table
nodos_larguerillos_table = convert_nodes_to_table_v2(nodos_larguerillos);


assignin('base', 'nodos_larguerillos_table', nodos_larguerillos_table);
...
bdf_filename = fullfile(databasePath,'\nodes.bdf')
write_bdf_points_v2(bdf_filename, nodes);
...
bdf_filename = fullfile(databasePath,'\lines_v5.bdf')
write_bdf_lines_v3(bdf_filename, lines_updated, material_info, property_info);
...
bdf_filename = fullfile(databasePath,'quads_all_v1.bdf')
write_bdf_quads_v1(bdf_filename, quads_all_3D_processed, [], pshell_info, material_info);
...
bdf_filename = fullfile(databasePath,'\tri_v1.bdf');
write_bdf_tris_v1(bdf_filename, tri_all_3D_processed, pshell_info, material_info);
...
bdf_filename = fullfile(databasePath,'\BC_v1.bdf');
generate_boundary_conditions_bdf(bdf_filename, root_nodes, root_front_spar_intrados, root_rear_spar_intrados, rib_fuselage_nodes);
...
bdf_filename = fullfile(databasePath,'\forces.csv');
export_forces_to_csv(bdf_filename, [forces_front;forces_rear] );
...
bdf_filename = fullfile(databasePath,'_mesh_',timestamp);
save_project_data(bdf_filename, ...
                  'lines', lines, ...
                  'quads', quads_all_3D, ...
                  'tri', tri_all_3D, ...
                  'nodes', nodes, ...
