function A_Ls = optimize_stringers_2(My, tss, material)
    % =============================================================
    % 📌 Function: optimize_stringers (Fixed Version)
    % =============================================================
    ...
    beff = min(max(30 * tss, 0.05), 0.2);  % Ensure 5 cm min, 20 cm max
    ...
    % 🔹 Modify force estimation to avoid direct tss dependency
    N_required = My ./ (beff / 2); 

    % ✅ Compute Required Stringer Area
    A_Ls = max(N_required ./ sigma_cr, 0.0001);  % Ensure a minimum of 0.1 cm²
