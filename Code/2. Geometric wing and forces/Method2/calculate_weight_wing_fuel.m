function results = calculate_weight_wing_fuel(ala, avion, rib_prop,n)
    ...
    R_i.anterior = squeeze(ala.costillas(:, :, end));  % anterior spar intersections
    R_i.posterior = squeeze(ala.costillas(:, :, 1));   % posterior spar intersections
    ...
    peso_ala =  avion.datosEstructural.porcentaje_peso_ala_MTOW * MTOW;          % Wing weight = 10% of MTOW
    peso_combustible =  avion.datosEstructural.porcentaje_peso_combustible_MTOW * MTOW;    % Fuel weight = 20% of MTOW
    ...
    W_ala_internal = W_ala_full;
    W_comb_internal = W_comb_full;
    assignin('base', 'W_ala_internal', W_ala_internal);
    assignin('base', 'W_comb_internal', W_comb_internal);
    ...
    size_L = size(W_ala_internal,1)-1;
    ...
    % Preallocate arrays for the mass wing force splitting at each rib intersection.
    h_A = zeros(size_L, 1);
    h_R = zeros(size_L, 1);
    l1_A = zeros(size_L, 1);
    l2_A = zeros(size_L, 1);
    ...
    for i = 1:size_L
        ...
        h_A(i) = norm(R_i.eje(i,:) - coord_interseccion_paralela_costillas_pasa_por_A(i, :, 1));
        h_R(i) = norm(R_i.eje(i,:) - coord_interseccion_paralela_costillas_pasa_por_A(i, :, 2));
    
        % Compute distances along the rib surface (from costilla points)
        l1_A(i) = norm(ala.costillas(i, :, end) - coord_interseccion_paralela_costillas_pasa_por_A(i, :, 1));
        l2_A(i) = norm(ala.costillas(i+1, :, end) - coord_interseccion_paralela_costillas_pasa_por_A(i, :, 1));
        l1_R(i) = norm(ala.costillas(i, :, 1) - coord_interseccion_paralela_costillas_pasa_por_A(i, :, 2));
        l2_R(i) = norm(ala.costillas(i+1, :, 1) - coord_interseccion_paralela_costillas_pasa_por_A(i, :, 2));
    
        % Distribute the mass force F_mass(i) into upper and lower components based on relative distances:
        P_A_masica_ala(i) = W_ala_internal(i) * h_R(i) / (h_A(i) + h_R(i));
        P_R_masica_ala(i) = W_ala_internal(i) * h_A(i) / (h_A(i) + h_R(i));
        ...
        % Distribute the mass force F_mass(i) into upper and lower components based on relative distances:
        P_A_masica_comb(i) = W_comb_internal(i) * h_R(i) / (h_A(i) + h_R(i));
        P_R_masica_comb(i) = W_comb_internal(i) * h_A(i) / (h_A(i) + h_R(i));
